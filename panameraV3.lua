...
-- НАСТРОЙКИ
-- ============================================
local soundId = "140207837688369"  -- ID звука
local volume = 1                    -- Громкость (0-1)
local speed = 1                     -- Скорость воспроизведения
...
local function PlayGrabSound()
    ...
    -- Родитель (можно в Backpack или в Character)
    local parent = player:FindFirstChild("Backpack")
    if not parent then
        parent = player:FindFirstChild("Character")
    end
    if not parent then
        parent = workspace -- если ничего нет, кидаем в workspace
    end
...
-- ============================================
-- Просто вызывай эту функцию когда нужно:
-- PlayGrabSound()

-- Пример: при захвате игрока
workspace.ChildAdded:Connect(function(Child)
    if Child.Name == "GrabParts" then
        local GrabPart = Child:FindFirstChild("GrabPart")
        if GrabPart then
            local WeldConstraint = GrabPart:FindFirstChild("WeldConstraint")
            if WeldConstraint and WeldConstraint.Part1 and 
               WeldConstraint.Part1.Parent and 
               WeldConstraint.Part1.Parent:FindFirstChild("Humanoid") then
                PlayGrabSound()
            end
        end
    end
